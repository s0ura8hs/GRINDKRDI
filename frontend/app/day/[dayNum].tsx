...
import {
  saveProgress, ensureDay, isDayComplete,
  getCurrentDayId, parseDurationSeconds,
} from '../../src/utils/storage';

export default function DayDetailScreen() {
  const { dayNum } = useLocalSearchParams<{ dayNum: string }>();
  ...
  const dayNumInt = parseInt(dayNum || '1', 10);
  ...
  const day = days.find((d) => d.dayNum === dayNumInt);
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={{ padding: 24 }}>
          ...
          <Text style={styles.dayTitle}>Day not found</Text>
        </View>
      </SafeAreaView>
    );
  }

  const status = getDayStatus(days, progress, streamKeys, day.id);
  const dayProg = ensureDay(progress, day.id, streamKeys);
  const allDone = isDayComplete(progress, day.id, streamKeys);
  ...
  const handleUploadProof = async (streamKey: string) => {
    ...
    if (!result.canceled && result.assets[0].base64) {
      ...
      ensureDay(newProgress, day.id, streamKeys).tasks[streamKey].proof = base64;
  ...
  const handleMarkDone = async (streamKey: string) => {
    ...
    const newProgress = { ...progress };
    const dp = ensureDay(newProgress, day.id, streamKeys);
    ...
    setTimers((prev) => { const next = { ...prev }; delete next[streamKey]; return next; });
    reload();
  };
      ...
      const id = setInterval(() => {
        setTimers((prev) => {
          ...
          if (!cur || cur.remaining <= 1) {
            clearInterval(intervalRefs.current[streamKey]);
            delete intervalRefs.current[streamKey];
            Alert.alert('Timer Done!', `Time's up!`);
  ...
  const resetTimer = (streamKey: string) => {
    if (intervalRefs.current[streamKey]) {
      clearInterval(intervalRefs.current[streamKey]);
      delete intervalRefs.current[streamKey];
    }
  ...
  return (
    <SafeAreaView style={styles.container} testID="day-detail-screen">
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.topRow}>
          ...
          <View style={styles.statusBadge}>
            <Text style={[
              styles.statusText,
              status === 'complete' && { color: Colors.successDark },
              status === 'backlog' && { color: Colors.danger },
              status === 'active' && { color: Colors.textPrimary },
              status === 'locked' && { color: Colors.lockedText },
            ]}>
              {status === 'complete' ? '✅ COMPLETE' : status === 'backlog' ? '⚠️ BACKLOG' : status === 'active' ? '▶ IN PROGRESS' : '🔒 LOCKED'}
            </Text>
          </View>
        </View>

        <View style={styles.dayHeader}>
          <View style={styles.dayBadge}><Text style={styles.dayBadgeText}>DAY {day.dayNum}</Text></View>
          <Text style={styles.dayTitle}>{day.monthTitle}</Text>
          <Text style={styles.daySubtitle}>Month {day.monthNum} · Week {day.weekNum}</Text>
        </View>

        {isLocked && (
          <View style={styles.lockedCard} testID="locked-overlay">
            <Text style={styles.lockedIcon}>🔒</Text>
            <Text style={styles.lockedTitle}>Day Locked</Text>
            <Text style={styles.lockedDesc}>Complete previous days first to unlock this day.</Text>
          </View>
        )}
        ...
        {!isLocked && roadmap.streams.map((stream, idx) => {
          ...
          return (
            <View key={stream.key} style={[styles.taskCard, tp.done && styles.taskCardDone]} testID={`task-card-${stream.key}`}>
              ...
              <Text style={styles.topicText}>{day.topics[stream.key] || '—'}</Text>
              ...
              {!tp.done && (
                <View style={styles.timerRow}>
                  ...
                  {timer && (
                    <TouchableOpacity style={styles.timerReset} onPress={() => resetTimer(stream.key)} testID={`timer-reset-${stream.key}`}>
...
const styles = StyleSheet.create({
  ...
  statusBadge: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 12, backgroundColor: Colors.surface, borderWidth: 1.5, borderColor: Colors.border },
  ...
  lockedCard: { backgroundColor: Colors.locked, borderRadius: 24, padding: 32, alignItems: 'center', borderWidth: 2, borderColor: Colors.border, marginBottom: 20 },
  lockedIcon: { fontSize: 48, marginBottom: 12 },
  lockedTitle: { fontSize: 18, fontWeight: '800', color: Colors.textPrimary, marginBottom: 6 },
